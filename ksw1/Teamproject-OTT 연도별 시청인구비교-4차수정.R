...
axis(1, at=1:3, labels = c('2019', '2020', '2021'))
lines(long.age10to19$pop,type='o',col=1,pch=0)
lines(long.age20to29$pop,type='o',col=1,pch=2)
lines(long.age30to39$pop,type='o',col=1,pch=3)
lines(long.age40to49$pop,type='o',col=1,pch=4)
lines(long.age50to59$pop,type='o',col=1,pch=5)
lines(long.age60to69$pop,type='o',col=1,pch=6)
lines(long.age70u$pop,type='o',col='red',pch=7, lty = 'dashed')

legend("topright", c("만10대 미만", "만10세-19세", "만20세-29세", "만30-39세", "만40-49세", "만50-59세", "만60-69세", "만70세 이상"), 
       pch = c(1,0,2,3,4,5,6,7), col = c(1,1,1,1,1,1,1,'red'),lty=1)
...
G <- ggplot(long.m.ages5, aes(times, percent)) + xlab("만50-59세") + geom_col(fill=color2)+ scale_fill_manual(values=color)+
  theme(axis.text.x=element_text(angle=90, hjust=1)) + 
  scale_x_discrete(limits = c('manytimes.day', 'one.day', 'week.5-7', 'week.3-4', 'week1-2', 'month.1-3','month.U1', 'year.1'))
